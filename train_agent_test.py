...
def run_process(data, agent, show_details=False):
    ...
    dist2 = 0
    return [obj_id, agent.positionX(), agent.positionY(), obj.positionX(),
...
def test_model(path):
    ...
    for data in dataset:
        ...
        data2.append(data+[f])
        fitness.append(f)
        logger.info('value for "{}" is {}'.format(o, f))

    logger.info(data2)
    logger.info(fitness)
    logger.info("median = {}".format(np.median(fitness)))
    logger.info("mean = {}".format(np.mean(fitness)))
...
if __name__ == "__main__":
    ...
    # test_model("models/model_0.ns")
    test_model("categorize.ns")
